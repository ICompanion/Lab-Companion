...
analyseController.getAll = function(callback) {
  bddController.executeQuery('select * from analyses', '', function(result, state){
      callback(data, state);
  });
};

analyseController.getByCode = function(code, callback) {
  bddController.executeQuery('select * from analyses where code_analyse = $1', [code], function(result, state){
      callback(data, state);
  });
};

analyseController.getAnalyse = function(code_analyse, callback) {
  bddController.executeQuery('select * from analyses where code_analyse = $1', [code_analyse], function(result, state){
      callback(data, state);
  });
};

analyseController.getPatientAnalysesList = function(patient_id, callback) {
    bddController.executeQuery('select code_analyse, date_analyse from public.analyse where patient_id = $1', [patient_id], function(result, state){
        callback(data, state);
    });
};
