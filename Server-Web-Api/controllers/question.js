...
questionController.getAll = function(callback) {
    bddController.executeQuery('select * from question', '', function(result, state){
        callback(data, state);
    });
};

questionController.getById = function(id, callback) {
    bddController.executeQuery('select * from question where id = $1', [id], function(result, state){
        callback(data, state);
    });
};
