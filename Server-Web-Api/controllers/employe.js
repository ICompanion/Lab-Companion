...
employeController.getAll = function(callback) {
  bddController.executeQuery('select * from employe', '', function(result, state){
      callback(data, state);
  });
};

employeController.getByIdentifiant = function(identifiant, callback) {
  bddController.executeQuery('select * from employe where identifiant = $1', [identifiant], function(result, state){
      callback(data, state);
  });
};

employeController.getByName = function(name, callback) {
  bddController.executeQuery('select * from employe where nom = $1 or prenom = $1', [name], function(result, state){
      callback(data, state);
  });
};

employeController.getById = function(id, callback) {
  bddController.executeQuery('select * from employe where id = $1', [id], function(result, state){
      callback(data, state);
  });
};
