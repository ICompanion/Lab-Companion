...
factureController.getAll = function(callback) {
  bddController.executeQuery('select * from facture', '', function(result, state){
      callback(data, state);
  });
};

factureController.getById = function(id, callback) {
  bddController.executeQuery('select * from facture where id = $1', [id], function(result, state){
      callback(data, state);
  });
};

factureController.getByPatientId = function(id, callback) {
  bddController.executeQuery('select * from facture where patient_id = $1', [id], function(result, state){
      callback(data, state);
  });
};
