...
etudeController.getAll = function(callback) {
    bddController.executeQuery('select distinct code_etude, nom_etude, date, description_etude from etudes_reponses', '', function(result, state){
        callback(data, state);
    });
};

etudeController.getByCode = function(code, callback) {
    bddController.executeQuery('select distinct code_etude, nom_etude, date, description_etude from etudes_reponses where code_etude = $1', [code], function(result, state){
        callback(data, state);
    });
};

etudeController.getByPatientId = function(code, callback) {
    bddController.executeQuery('select * from participations where identifiant = $1', [code], function(result, state){
        callback(data, state);
    });
};

etudeController.getAnswers = function(code, callback) {
    bddController.executeQuery('select * from propositions where id_question = $1', [code], function(result, state){
        callback(data, state);
    });
};
etudeController.getQuestions = function(code, callback) {
    bddController.executeQuery('select * from questions where code_etude = $1', [code], function(result, state){
        callback(data, state);
    });
};

etudeController.getAllAnswers = function(callback) {
    bddController.executeQuery('select code_etude, question, reponse, nbr_reponses from etudes_reponses', '', function(result, state){
        callback(data, state);
    });
};
